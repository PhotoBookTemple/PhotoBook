...
$media_url_prefix = "http://cis-linux2.temple.edu/~tuf77221/download_file.php?filename=";
...
if (isset($_POST["data"])) {
	...
	while ($rows = mysqli_fetch_array($result)) { 
			$photoURL = $media_url_prefix . $rows[3];
			$response .= "{\"photoID\":\"" . $rows[0] . "\",\"photoName\":\"" . $rows[3] . "\",\"photoPath\":\"" . $photoURL ."\",\"photoCaption\":\"" . $rows[6] . "\",\"timeStamp\":\"" . $rows[4] ."\",\"gpsLocation\":\"" . $rows[7] ."\",\"locAltitude\":\"" . $rows[8] ."\",\"locTemp\":\"" . $rows[9] . "\"},";
	}
